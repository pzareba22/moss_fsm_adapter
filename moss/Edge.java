...
public class Edge implements Comparable<Edge>, Serializable {
   ...
   *  @since  2015.05.02 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public int hashCode ()
  {                             /* --- compute the Hash code */
    int h, t, u;                /* hash code and buffers */

    t  = (this.src.type & ~Node.CHAIN) ^ this.src.deg;
    u  = (this.dst.type & ~Node.CHAIN) ^ this.dst.deg;
    t  = t + u + (t & u) + (t | u) + (t ^ u);
    h  = t ^ (t << 11) ^ (t << 19);
    t += this.type;             /* combine node types and degrees */
    h ^= t ^ (t <<  7) ^ (t << 17);
