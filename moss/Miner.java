/*----------------------------------------------------------------------
  File    : Miner.java
  Contents: Find frequent/discriminative substructures of graphs
  Author  : Christian Borgelt
  History : 2002.03.11 file created
            2002.03.15 main function added
            2002.03.18 search improved, output to files added
            2002.03.21 function report added, main program restructured
            ...
            2011.08.10 check for excluded edges added to duplicate()
            2013.04.19 bug concerning default excluded node types fixed
            2014.10.23 changed from LGPL license to MIT license
            2016.07.05 Kekule representations not converted by default
            2020.10.04 graph association rule mining added (internal)
            2020.10.05 graph association rule writing added
            2020.10.06 output of association rules extended
            2020.10.11 rule output enhanced by two lift values
            2021.10.10 optional handling of directed edges added
----------------------------------------------------------------------*/
package moss;
...
class RepoElem implements Serializable {
  ...
  protected RepoElem (Fragment frag)
...
/*--------------------------------------------------------------------*/
/** Class for managing graph patterns (for rule generation).
 *  <p>A graph pattern is a list element that represents
 *  a substructure found (and reported) in the search.</p>
 *  @author Christian Borgelt
 *  @since  2020.10.03 */
/*--------------------------------------------------------------------*/
class GraphPattern implements Serializable {
  ...
  /*  instance variables                                              */
  /*------------------------------------------------------------------*/
  /** the graph pattern identifier (running count, starting with 1) */
  protected int          id;
  /** the graph referred to */
  protected Graph        graph;
  /** the support of the substructure (in the focus) */
  protected int          supp;
  /** the next element in the graph pattern list */
  protected GraphPattern succ;

  /*------------------------------------------------------------------*/
  /** Create a graph pattern.
   *  @param  frag the fragment to store
   *  @since  2020.10.03 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected GraphPattern (int id, Fragment frag)
  {                             /* --- create a graph pattern */
    this.id    = id;
    this.graph = frag.getGraph();
    this.supp  = frag.supp[0];
  }  /* GraphPattern() */
...
public class Miner implements Runnable, Serializable {
  ...
  private static final long serialVersionUID = 0x00080003;
  ...
  public  static final int CLOSED     = 0x00001000;
  /** flag for filtering open rings */
  public  static final int CLOSERINGS = 0x00002000;
  /** flag for merging ring extensions with the same first edge */
  public  static final int MERGERINGS = 0x00004000;
  /** flag for full (unmerged) ring extensions */
  private static final int FULLRINGS  = 0x00008000;
  /** flag for pruning fragments with unclosable rings */
  public  static final int PR_UNCLOSE = 0x00010000;
  /** flag for partial perfect extension pruning */
  public  static final int PR_PARTIAL = 0x00020000;
  /** flag for full perfect extension pruning */
  public  static final int PR_PERFECT = 0x00040000;
  /** flag for equivalent sibling extension pruning */
  public  static final int PR_EQUIV   = 0x00080000;
  /** flag for canonical form pruning */
  public  static final int PR_CANONIC = 0x00100000;
  /** flag for unembedding siblings of the current search tree nodes */
  public  static final int UNEMBED    = 0x00200000;
  /** flag for normalized substructure output */
  public  static final int NORMFORM   = 0x00400000;
  /** flag for verbose reporting */
  public  static final int VERBOSE    = 0x00800000;
  /** flag for converting Kekul&eacute; representations */
  public  static final int AROMATIZE  = 0x01000000;
  /** flag for conversion to another description format */
  public  static final int TRANSFORM  = 0x02000000;
  /** flag for conversion to logic representation */
  public  static final int LOGIC      = 0x04000000;
  /** flag for no search statistics output */
  public  static final int NOSTATS    = 0x08000000;
  /** default search mode flags: edge extensions, embeddings,
   *  canonical form and full perfect extension pruning */
  public  static final int DEFAULT    = EDGEEXT         | PR_CANONIC
                                      | ORBITS | CLOSED | PR_PERFECT;
  /* internal flag for found source/destination node */
  private static final int FOUND      = 0x80000000;
   ...
   *  @param  level  the level at which to switch to embeddings
   *  @param  maxepg the maximum number of embeddings per graph
   ...
   *  @throws IOException if writing the log file failed
   *  @since  2006.06.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- set the excluded node types */
    ...
    Notation inp = this.configNtns();

    if      (!ntn.hasFixedTypes())
      ntn.setTypeMgrs(inp);     /* create and configure notation */
    if ((extype == null) || extype.equals(""))
      this.extype = null;       /* if no excluded node types given, */
    else if (ntn.hasFixedTypes() && !inp.hasFixedTypes()) {
      this.log.println("warning: seed/excluded types format mismatch!");
      this.log.println("warning: discarding excluded node types!");
      this.extype = null; }     /* check for consistent formats */
    else {                      /* if everything is consistent */
    ...
    if ((exseed == null) || exseed.equals(""))
      ...
      this.exseed = null; }     /* check for consistent formats */
    else {                      /* if everything is consistent */
   ...
   *  @throws IOException if the seed is not connected
   *  @since  2006.06.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
   ...
   *  @throws IOException if the seed is not connected
   *                      or writing the log file failed
   *  @since  2006.06.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- set the seed structure */
    ...
    if      (!ntn.hasFixedTypes())   /* create and configure notation */
      ...
      this.seed = null; return; /* check for consistent formats */
    }                           /* and parse the seed description */
   ...
   *  @throws IOException if creating the input reader failed
   *  @since  2007.03.05 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  { this.writer = writer; this.wrids = null; this.wrules = null; }
  ...
  { this.writer = writer; this.wrids = wrids; this.wrules = null; }

  /*------------------------------------------------------------------*/
  /** Set the output writers.
   *  @param  writer the writer to write the found substructures
   *  @param  wrids  the writer to write the graph identifiers
   *  @param  wrules the writer to write the graph rules
   *  @since  2020.10.05 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public void setOutput (GraphWriter writer, Writer wrids,Writer wrules)
  { this.writer = writer; this.wrids = wrids; this.wrules = wrules; }

  /*------------------------------------------------------------------*/
  /** Set the output writer.
   *  @param  fname the name of the file for the found substructures
   *  @param  format the format for the output
   *  @throws IOException if creating the output writer failed
   ...
   *  @param  format the format for the output
   *  @throws IOException if creating an output writer failed
   *  @since  2006.06.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
   ...
   *  @param  fn_sub   the name of the file for the found fragments
   *  @param  fn_ids   the name of the file for the graph identifiers
   *  @param  fn_rules the name of the file for the graph rules
   *  @param  format the format for the output
   *  @throws IOException if creating an output writer failed
   *  @since  2006.06.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public void setOutput (String fn_sub, String format,
                         String fn_ids, String fn_rules)
    throws IOException
  {                             /* --- set the output writers */
    this.writer = GraphWriter.createWriter(new FileWriter(fn_sub),
                    GraphWriter.SUBS, format);
    this.wrids  = ((fn_ids   != null) && !fn_ids.equals(""))
                ? new FileWriter(fn_ids)   : null;
    this.wrules = ((fn_rules != null) && !fn_ids.equals(""))
                ? new FileWriter(fn_rules) : null;
  }  /* setOutput() */

  /*------------------------------------------------------------------*/
  /** Set the canonical form.
   *  @param  cnf the canonical form to set
  ...
  {                             /* --- configure notations */
    ...
    Notation inp = (this.graphs != null)
                 ?  this.graphs.getNotation()
                 :  this.reader.getNotation();
    if      (!out.hasFixedTypes())  /* if output types are not fixed, */
      out.setTypeMgrs(inp);     /* transfer the input types */
    else if (!inp.hasFixedTypes())  /* if input types are not fixed, */
      inp.setTypeMgrs(out);     /* set the fixed types for the input */
    return inp;                 /* return the configured notation */
  }  /* configNtns() */
  ...
  {                             /* --- convert Kekul\'e to aromatic */
    ...
    for (NamedGraph g = this.graphs; g != null; g = g.succ)
      if (BondTypeMgr.aromatize(g) > 0) n++;
  ...
  {                             /* --- reorganize the repository */
    ...
    RepoElem[] b;               /* buffer for old hash bin array */
    RepoElem   e;               /* to traverse the hash bin lists */

    b = this.bins;              /* note the old hash bin array and */
    k = (b.length << 1) +1;     /* compute the new hash table size */
    this.bins = new RepoElem[k];/* allocate a new hash bin array */
    for (i = b.length; --i >= 0; ) {
      while (b[i] != null) {    /* traverse the nonempty bins */
        e = b[i]; b[i] = e.succ;/* of the old hash bin array */
  ...
  /*------------------------------------------------------------------*/
  /** Check whether a given fragment has already been processed.
  ...
  private boolean isInRepo (Fragment frag)
  {                             /* --- check for a duplicate fragment */
    int       k;                /* loop variable */
    int       hash, i;          /* hash code and hash bin index */
    RepoElem  re;               /* to traverse the bin list */
    ...
    for (re = this.bins[i]; re != null; re = re.succ) {
      this.fragcmp++;           /* traverse the hash bin list */
      ...
      for (k = re.edges.length; --k >= 0; ) {
        if (re.edges[k].mark <= Integer.MIN_VALUE) break;
        re.edges[k].mark = 0;   /* mark the substructure unless */
      }                         /* an edge is marked as excluded */
      if (k >= 0) {             /* if there was an excluded edge */
        while (++k < re.edges.length) re.edges[k].mark = -1;
        continue;               /* unmark the marked edges */
      }                         /* and skip the embedding */
      /* Due to the way in which the repository is used, especially,  */
      /* because it is cleared for every new start node label, it is  */
      /* impossible that an edge of a stored fragment is marked with  */
      /* Integer.MIN_VALUE. However, for general safety and possible  */
      /* other uses of this function, the above check has been added. */
      while ((emb != null) && (emb.graph == re.graph)) {
        this.embcmp++;          /* count the embedding */
  ...
  /*------------------------------------------------------------------*/
  /** Add a substructure to the repository.
   *  <p>The hash table of the substructure repository is emptied.</p>
   *  @param  frag the fragment/substructure to add
   *  @since  2011.08.10 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  private void addToRepo (Fragment frag)
  {                             /* --- add a substructure */
    int      i;                 /* hash bin index */
    RepoElem re;                /* created repository element */

    re = new RepoElem(frag);    /* create a new repository element */
    i  = frag.hashCode() % this.bins.length;
    re.succ = this.bins[i];     /* compute index and add the element */
    this.bins[i] = re;          /* at the head of the hash bin list */
    if (++this.recnt > this.bins.length)
  ...
  {                             /* --- report a substructure */
    int          id;            /* substructure identifier */
    Graph        sub;           /* fragment as a graph */
    GraphPattern pat;           /* graph pattern for storing */
    ...
    if ((this.epmgr != null)    /* add single edges as edge patterns */
    &&  (frag.list.edges.length == 1))
      this.epmgr.add(frag.list.edges[0], frag.supp[0]);
    ...
    &&  !frag.isClosed(this.cnf)) {   /* skip non-closed fragments */
    ...
    /* --- store in repository (for rules) --- */
    if (this.wrules != null) {  /* if to write association rules */
      pat       = new GraphPattern(this.subcnt, frag);
      pat.succ  = this.pats;    /* create a graph pattern and */
      this.pats = pat;          /* add it to the result pattern list */
    }                           /* (rules ate written at the end) */
    ...
    if ((this.mode & NORMFORM) != 0) {
      ...
      if (this.norm == null) this.norm = new CnFBreadth1();
  ...
  {                             /* --- recursive search with embeds. */
    ...
    if (depth > this.maxdepth)  /* update the maximal depth */
      this.maxdepth = depth;    /* of the recursion/search tree */
    ...
    if ((this.mode & VERBOSE) != 0) {
      ...
      k = 0;                    /* init. the embedding counter */
      for (emb = frag.firstEmb(); emb != null; emb = frag.nextEmb()) {
        ...
        if (k > 0)              /* if more than one embedding, */
          System.out.print(":" +k +" "); /* print their number */
        k = 1;                  /* reinit. the embedding counter */
        g = emb.graph;          /* note the new graph */
    ...
    while (cnt > n)             /* "delete" the unused fragments */
    ...
    &&  ((this.mode & (PR_PERFECT|PR_PARTIAL)) != 0)) {
      for (i = 0; i < cnt; i++) /* search for a perfect extension */
        if (xfs[i].isPerfect(chain)) break;
    ...
    &&  ((this.mode & PR_EQUIV) != 0)) {
      ...
      for (i = n = 1; i < cnt; i++) {
        if (!xfs[i].equivSiblings()) {   /* collect frags. without */
          xfs[n++] = xfs[i]; continue; } /* equivalent siblings */
    ...
    /* --- remove duplicates with repository --- */
      ...
      for (i = 0; i < cnt; i++){/* traverse the fragments */
        if (this.isInRepo(xfs[i])) {
          this.duplic++; continue; }
        this.addToRepo(xfs[i]); /* identify, count, and remove */
        xfs[n++] = xfs[i];      /* already processed fragments */
      }                         /* and collect the rest */
    }
    ...
    this.report(frag);          /* report the current fragment */
    if (Thread.currentThread().isInterrupted())
      this.stop = true;         /* check for thread interruption */
  ...
  {                             /* --- search for substructures */
    ...
    if ((this.mode & PR_CANONIC) == 0) {
      ...
      this.recnt = 0;           /* if no canonical form pruning, */
    ...
    this.chains  = this.invalid = this.repcnt  = this.fragcmp = 0;
    this.isocnt  = this.embcmp  = 0;
    this.writer.writeHeader();  /* print header for substructures */
    if (this.wrids  != null)    /* and for graph identifier lists */
  ...
  {                             /* --- recursive search w/o embeds. */
    ...
    while (true) {              /* extension generation loop */
      ...
      if (x == null) break;     /* if there is none, abort the loop */
      if (cnt >= size) {        /* if the fragment array is full */
    ...
    for (i = 0; i < cnt; i++) { /* traverse the fragments */
      ...
      if (xfs[i].supp[0] < this.supp) {  /* check fragment support */
        this.lowsupp++; continue; }      /* against minimum support */
      xfs[n++] = xfs[i];        /* collect the frequent fragments */
    ...
    for (i = 0; i < cnt; i++) { /* traverse the remaining fragments */
      if ((depth < this.emblvl ? !this.recPlain(xfs[i], depth)
                               : !this.recEmbed(xfs[i], depth)))
  ...
  {                             /* --- search for substructures */
    ...
    ExtMgr     xemgr;           /* manager for extension edges */
    TypeMgr    ndmgr;           /* manager for node types and names */
    String     s;               /* buffer for output formatting */
    ...
    xemgr = new ExtMgr(this.graphs.dir, this.coder.size());
    for (graph = this.graphs; graph != null; graph = graph.succ) {
      if (graph.group != 0) break;    /* traverse the focus graphs */
      for (i = graph.edgecnt; --i >= 0; ) {
        e = graph.edges[i];     /* traverse all edges of each graph */
        if (e.mark >= -1) xemgr.add(e);
    ...
    this.cnf.setExtMgr(xemgr);  /* the extension edge manager */
    if ((this.mode & PR_CANONIC) == 0) {
      this.bins  = new RepoElem[1023];
      this.recnt = 0;           /* if no canonical form pruning, */
    }                           /* create a substructure repository */
    ...
    if (this.seed != null) {    /* if there is a seed structure, */
      ...
      for (i = 0; i < this.coder.size(); i++) {
        ...
        if (this.bins != null)  /* clear the repository */
          Arrays.fill(this.bins, null);
        g = new Graph(this.graphs.ntn, this.coder, this.graphs.dir);
   ...
   *  @throws IOException if writing the graphs failed
   *  @since  2002.03.11 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- init. a substructure miner */
    ...
    String  rulefn = null;      /* name of graph rule file */
    ...
    double  pconf  = 80.0;      /* minimum confidence of a rule */
    ...
    Notation      nota;         /* buffer for graph notation */
    ...
    if (args.length > 0) {      /* if no arguments are given */
      ...
      System.out.println(" (default: " +pconf +"%)");
    ...
    /* remaining option characters: F I V W X Y */
    ...
    for (i = 0; i < args.length; i++) {
      ...
      &&  (s.charAt(0) == '-')) {
        ...
        switch (s.charAt(1)) {  /* evaluate option */
          ...
          case 'D': smode |=  DIRECTED;                          break;
          case 'C': smode &= ~CLOSED;                            break;
          case 'Z': rulefn = s.substring(2);                     break;
          ...
          case 'w': pconf  = Double.parseDouble(s.substring(2)); break;
          case 'k': stype  = Integer.parseInt(s.substring(2))
                          |  (stype & Fragment.GREEDY);          break;
          case 'G': stype &= ~Fragment.GREEDY;                   break;
          case 'a': matom &= ~AtomTypeMgr.AROMATIC;              break;
          case 'c': matom &= ~AtomTypeMgr.CHARGEMASK;            break;
          case 'd': matom &= ~AtomTypeMgr.ELEMMASK;
                    mrgat &= ~AtomTypeMgr.ELEMMASK;              break;
          case 'J': mrgat &= ~AtomTypeMgr.ELEMMASK;              break;
          case ':': mbond &=  BondTypeMgr.DOWNGRADE;
                    mrgbd &=  BondTypeMgr.DOWNGRADE;             break;
          case 'b': mbond &=  BondTypeMgr.SAMETYPE;
                    mrgbd &=  BondTypeMgr.SAMETYPE;              break;
          case 'B': mrgbd &=  BondTypeMgr.SAMETYPE;              break;
          ...
          case 'A': smode |= ALLEXTS;                            break;
          case 'Q': smode |= CLASSES;                            break;
          case 'g': cfname = s.substring(2);                     break;
          case 'P': smode &= ~PR_PARTIAL;                        break;
          case 'p': smode &= ~PR_PERFECT;                        break;
          case 'e': smode &= ~PR_EQUIV;                          break;
          case 'q': smode &= ~PR_CANONIC;                        break;
          case 'h': smode &= ~ORBITS;                            break;
          case 'u': level  = Integer.parseInt(s.substring(2));   break;
          case 'M': maxepg = Integer.parseInt(s.substring(2));   break;
      ...
      &&       (s.charAt(0) == '+')) {
        ...
        switch (s.charAt(1)) {  /* evaluate option */
          ...
          case 'd': matom |= AtomTypeMgr.ELEMMASK;
                    mrgat |= AtomTypeMgr.ELEMMASK;               break;
          case 'D': mrgat |= AtomTypeMgr.ELEMMASK;               break;
          case ':': mbond &= BondTypeMgr.UPGRADE;
                    mrgbd &= BondTypeMgr.UPGRADE;                break;
          case 'b': mbond |= BondTypeMgr.BONDMASK;
                    mrgbd |= BondTypeMgr.BONDMASK;               break;
          case 'B': mrgbd |= BondTypeMgr.BONDMASK;               break;
        ...
        }                       /* there should be one fixed args: */
      }                         /* name of the graph database file */
    }
    if (rulefn != null)         /* if to generate association rules, */
      smode &= ~CLOSED;         /* do not restrict to closed patterns */

    /* --- initialize input/output --- */
    if (( input.equals("nelist") &&  input.equals("nel"))
    ||  (output.equals("nelist") && output.equals("nel")))
      smode &= ~DIRECTED;       /* directed edges only for NEList */
    if ((smode & DIRECTED) != 0)    /* if directed edges, */
      smode &= ~(RINGEXT|CHAINEXT); /* no ring/chain extensions */
    this.setMode(smode);        /* set the search mode */
    this.setGrouping(split, invert);
    dir = ((smode & DIRECTED) != 0) ? GraphReader.DIRECTED : 0;
    rdr = GraphReader.createReader(new FileReader(datfn),
            GraphReader.GRAPHS|dir, input);
    if (rdr == null)            /* create the graph reader */
      throw new IOException("error: invalid input format " +input);
    this.setInput(rdr);         /* set the graph reader */
    wrt = GraphWriter.createWriter(new FileWriter(subfn),
            GraphWriter.SUBS, output);
    if (wrt == null)            /* create the graph writer */
      throw new IOException("error: invalid output format " +output);
    this.setOutput(wrt,         /* set the graph writer */
                   ((idsfn  == null) || idsfn.equals(""))
                   ? null : new FileWriter(idsfn),
                   ((rulefn == null) || rulefn.equals(""))
                   ? null : new FileWriter(rulefn));
    if ((smode & (TRANSFORM|LOGIC)) != 0)
      return;                   /* check for a mere transformation */

    /* --- initialize other search variables --- */
    nota = Notation.createNotation(format);
    if ((nota == null) || !nota.isLine())
      throw new IOException("error: invalid format " +format);
    this.setType(stype);            /* set the support type */
    if (psupp >= 0) psupp *= 0.01;  /* change support values from */
    if (pcomp >= 0) pcomp *= 0.01;  /* percentages to fractions */
    this.setLimits(psupp, pcomp);   /* set the support limits */
    pconf = Math.abs(pconf) *0.01;  /* turn confidence into fraction */
    this.setConf(pconf);            /* set the minimum confidence */
  ...
  {                             /* --- run substructure search */
    ...
    int        base;            /* base support (largest poss. supp.) */
    boolean    dir;             /* whether graphs are directed */
    ...
    &&  (this.cnts[0] <= 0))    /* check for graphs in focus */
    ...
    &&  (this.seed.nodecnt > 1)) {
      if (this.cnf instanceof CnFDepth) this.cnf = new CnFBreadth1();
    ...
    if (k != Fragment.GRAPHS) { /* if support is not number of graphs */
      ...
      this.emblvl = 0;          /* all fragments are reported */
    ...
    /* are not supported in this case. Embeddings are mandatory.      */
    if ((this.mode & CLOSED) == 0)
    ...
    if (((this.mode & CHAINEXT) != 0)
    &&  !(this.cnf instanceof CnFBreadth1))
    ...
    if ((this.mode & MERGERINGS) != 0) {
      this.mode &= ~UNEMBED; this.maxepg = 0; }
    ...
    /* --- prepare for rule generation --- */
    if (this.wrules != null) {  /* if to create association rules */
      if ((this.type & Fragment.SUPPMASK) == Fragment.GRAPHS)
        base = this.cnts[0];    /* if support is number of graphs, */
      else {                    /* get the number of graphs */
        base = 0;               /* if MIS, HO or MNI support */
        for (graph = this.graphs; graph != null; graph = graph.succ) {
          if (graph.group != 0) break;
          base += graph.nodecnt;/* traverse the graphs and */
        }                       /* sum the number of nodes */
      }                         /* (largest possible support) */
      dir = (this.mode & DIRECTED) != 0;
      k   = this.graphs.getNotation().getEdgeMgr().getTypeCount();
      this.epmgr = new EdgePatternMgr(dir, k, base);
    ...
    /* --- generate rules --- */
    if (this.wrules != null) this.rules();
  }  /* mine() */
  ...
  {                             /* --- generate (association) rules */
    int            i, m;        /* loop variable, (marker) buffer */
    int            k;           /* buffer for node/edge counts */
    int            n = 0;       /* number of found rules */
    int            e, emax;     /* (maximum) number of edges */
    GraphPattern[] pats;        /* patterns by size (num. of edges) */
    GraphPattern   p, q;        /* to traverse the graph patterns */
    Embedding      emb;         /* embedding (only return value) */
    int            base,  supp; /* buffers for support values */
    int            head1, head2;/* rule head support values */
    double         conf;        /* confidence of association rule */
    double         lift1, lift2;/* lift       of association rule */
    int            hdid = -1;   /* index of rule head edge */
    Edge           edge;        /* rule head edge (not in rule body) */
    Node           node;        /* to traverse the nodes */
    int            psrc, pdst;  /* indices of source and dest. nodes */
    int            qsrc, qdst;  /* in original and image graph */
    long           t;           /* for time measurements */

    t = System.currentTimeMillis();
    this.log.print("forming association rules ... ");
    this.wrules.write("part\tfull\t");
    if ((this.pats != null)     /* if NElist format is used */
    &&  (this.pats.graph.ntn instanceof NEList))
      this.wrules.write("src\tdst\tedge\tnode\t");
    this.wrules.write("base\tbody\tsupp\thead1\thead2\t"
                     +"conf\tlift1\tlift2\n");
    emax = 0;                   /* traverse the found patterns */
    for (p = this.pats; p != null; p = p.succ) {
      e = p.graph.getEdgeCount();
      if (e > emax) emax = e;   /* find the largest number of edges */
    }                           /* (maximum pattern size) */
    pats = new GraphPattern[emax+1];
    while (this.pats != null) { /* traverse the found patterns */
      p = this.pats; this.pats = p.succ;
      e = p.graph.getEdgeCount();
      p.succ = pats[e]; pats[e] = p;
    }                           /* sort patterns by edge count */
    qsrc = qdst = -1;           /* initialize node indices */
    for (e = 0; e < emax; e++){ /* traverse the patterns by size */
      for (p = pats[e]; p != null; p = p.succ) {
        for (q = pats[e+1]; q != null; q = q.succ) {
          /* The candidate full graph q must have exactly one edge */
          /* more than the candidate partial/antecedent graph p.   */
          /* This is ensured by taking the partial (or antecedent) */
          /* graph p from the patterns of size e and the full      */
          /* graph q from the patterns of site e+1 (# of edges).   */
          q.graph.mark(-1);     /* prepare patterns and try */
          p.graph.mark(-1);     /* to embed smaller into larger */
          emb = q.graph.markEmbed(p.graph);
          if (emb == null)      /* if no embedding into q found, */
            continue;           /* skip the potential pattern pair */
          /* An embedding marks the nodes of the full graph q with  */
          /* the indices of the nodes of the partial or antecendent */
          /* graph p that are mapped to them by the embedding.      */
          conf = q.supp /(double)p.supp;
          if (conf < this.conf) /* compute the rule confidence and */
            continue;           /* if below threshold, skip rule */
          n += 1;               /* count the found association rule */
          for (i = 0; i < q.graph.edgecnt; i++) {
            if (q.graph.edges[i].mark < 0) {
              hdid = i; break; }
          }                     /* find head edge (not in body p) */
          /* There must be exactly one edge in the full graph q     */
          /* that has not been marked, that is, that is not in the  */
          /* partial (or antecedent) graph p. This is the new edge, */
          /* which is also the consequent of the association rule.  */
          edge = q.graph.edges[hdid];
          psrc = edge.src.mark; /* get markers of incident nodes */
          pdst = edge.dst.mark; /* to the edge added by the rule */
          /* psrc and pdst hold the indices of the source and the */
          /* destination node in the partial/antecedent graph p.  */
          /* If the edge leads to a new node, one of them is -1,  */
          /* because this new node was not marked by embedding.   */
          if      (psrc < 0) {  /* if source      node not in body */
            for (i = 0; i < q.graph.nodecnt; i++)
              if (q.graph.nodes[i].mark < 0) { qsrc = i; break; } }
          else if (pdst < 0) {  /* if destination node not in body */
            for (i = 0; i < q.graph.nodecnt; i++)
              if (q.graph.nodes[i].mark < 0) { qdst = i; break; }
          }                     /* find image index of dest. node */
          /* If a new node was added, that is, if either psrc or pdst */
          /* are -1, there must be an unmarked node (mark < 0) in the */
          /* full graph q, which is this (new) destination node.      */
          /* The index of this new node is stored in qsrc or qdst.    */
          /* Note that the index of this node cannot be retrieved     */
          /* from the nodes incident to the new edge (index hdid),    */
          /* because the nodes do not "know" their indices.           */
          this.wrules.write(p.id +"\t" +q.id +"\t");
          if (q.graph.ntn instanceof NEList) {
            if (((this.mode & DIRECTED) == 0) && (psrc < 0))
                 this.wrules.write((pdst+1) +"\t" +(psrc+1) +"\t");
            else this.wrules.write((psrc+1) +"\t" +(pdst+1) +"\t");
            this.wrules.write((hdid+1) +"\t");
            if      (psrc < 0) this.wrules.write((qsrc+1) +"\t");
            else if (pdst < 0) this.wrules.write((qdst+1) +"\t");
            else               this.wrules.write("\t");
          }                     /* write additional information */
          base = this.epmgr.getSupp();
          this.wrules.write(base +"\t" +p.supp +"\t" +q.supp +"\t");
          /* Rule head support type 1: Support of edges with the same */
          /* edge type and incident node types in all given  graphs.  */
          head1 = this.epmgr.getSupp(edge);
          lift1 = conf / (head1 / (double)base);
          /* Rule head support type 2: Support of edges with the same */
          /* edge type and type of new node if there is a new node    */
          /* or just the support of edges with the same type if both  */
          /* incident nodes are in the partial/antecedent graph p.    */
          head2 = (psrc < 0) ? this.epmgr.getSuppSrc (edge)
                : (pdst < 0) ? this.epmgr.getSuppDest(edge)
                : this.epmgr.getSupp(edge.type);
          lift2 = conf / (head2 / (double)base);
          this.wrules.write(head1 +"\t" +head2 +"\t"
              +conf  +"\t" +lift1 +"\t" +lift2 +"\n");
        }                       /* write information about */
      }                         /* the found graph rule */
      this.wrules.flush();      /* to the output file */
    }                           /* finally write a log message */
    t = System.currentTimeMillis() -t;
    this.log.println("[" +n +" rule(s)] done [" +(t/1000.0) +"s].");
  }  /* rules() */
  ...
  {                             /* --- clean up after search */
    ...
    if (this.wrules != null) { this.wrules.close(); this.wrules = null;}
  }  /* term() */
