...
public class Fragment implements Serializable {
  ...
  /*  constants                                                       */
  /*------------------------------------------------------------------*/
  private static final long serialVersionUID = 0x00010005;
  ...
  /*------------------------------------------------------------------*/
  /** Create an empty fragment.
   *  @param  dir  whether the fragment is directed
  ...
  { this(graph, max, (graph != null) ? graph.dir : false); }

  /*------------------------------------------------------------------*/
  /** Create a fragment with an embedding limit from a graph.
   *  @param  graph the subgraph representing the fragment
   *  @param  max   the maximum number of embeddings per graph
   *  @param  dir   whether the graphs are directed
   *  @since  2002.03.11 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected Fragment (Graph graph, int max, boolean dir)
  {                             /* --- create an initial fragment */
    this.dir   = dir;           /* note the directionality */
    ...
    this.hash  = -1;            /* set the hash code invalid */
  ...
  {                             /* --- create an extended fragment */
    this.dir   = frag.dir;      /* copy the directionality */
   ...
   *  @param  reversed whether the edge direction is reversed
   *  @since  2010.01.23 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected Fragment (Fragment frag, int src, int dst,
                      int edge, int node, boolean reversed)
  {                             /* --- create an extended fragment */
    this(frag, frag.graph.edgecnt, src, dst);
    this.graph = new Graph(frag.graph, src, dst, edge, node, reversed);
  }  /* Fragment() */
  ...
  {                             /* --- create an extended fragment */
    ...
    this.dir   = cnf.frag.dir;  /* copy the directionality */
    this.graph = null;          /* not yet available as a graph */
  ...
  public int hashCode ()
  {                             /* --- compute a hash code */
    if (this.hash < 0)          /* compute hash code if unknown */
      this.hash = (this.graph != null)
    ...
    return this.hash;           /* return the hash code */
  }  /* hashCode() */
   ...
   *  @param  closed  whether to set or clear the closed flag
   ...
   *  @return whether the fragment is in canonical form
   *          -1, if the fragment differs from the canonical form
   *              in the fixed edges,
