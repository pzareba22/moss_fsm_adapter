/*----------------------------------------------------------------------
  ...
  History : 2002.03.11 file created as file submol.java
            ...
            2021.10.09 adapted plain    search mode to directed edges
            2021.10.10 adapted standard search mode to directed edges
            2022.11.16 rightmost path finding adapted to multigraphs
----------------------------------------------------------------------*/
package moss;
 ...
 *      <li>node attribute (ascending)</li></ul>
 ...
 *  Keep in mind that i<sub>s</sub> is sorted descendingly.
 *  @author Christian Borgelt
 *  @since  2005.08.11 */
/*--------------------------------------------------------------------*/
public class CnFDepth extends CanonicalForm {
  ...
  {                             /* --- init. a rightmost extension */
    ...
    Edge  e, p;                 /* to traverse/access the edges */
    ...
    this.pei  = -1;             /* start with the first edge */
    ...
    n = 0; p = null;            /* init. the number of path edges */
    ...
    while (s.mark > 0) {        /* construct rightmost path upwards */
      for (k = -1, i = s.deg; --i >= 0; ) {
        ...
        if ((d.mark >= k) && (d.mark < s.mark)) {
          /* If d.mark == k, there are a multiple edges (multigraph), */
          /* of which only the one with the smallest type is the edge */
          /* on the rightmost path, all others are "ring" edges.      */
          if ((d.mark > k) || (e.type < p.type))
            p = e;              /* find the adjacent node with the */
          k = d.mark;           /* largest index smaller than own, */
          p = e;                /* that is, find the parent node */
        }                       /* on the rightmost path and note */
      ...
      this.path[n++] = p.mark;  /* note the index of the next edge */
  ...
  {                             /* --- create the next extension */
    int    i;                   /* loop variable */
    Node   s, d, v;             /* to traverse/access the nodes */
    Node[] o, n;                /* to traverse/access the nodes */
    Edge   e, x;                /* to traverse/access the edges */
    int    t, u;                /* node type or edge code buffer */
    ...
    while (true) {              /* find the next unprocessed edge */
      while (++this.idx >= s.deg) {
        ...
        if (this.src > this.frag.dst) /* if beyond rightmost leaf, */
          ...
          e = this.emb.edges[this.path[++this.pei]];
        ...
        /* Such nodes have been added by a perfect extension (if      */
        /* there were search tree branches to the left of its branch) */
        /* or a ring extension (if more than one node has been added).*/
      ...
      if (d.mark < 0) {         /* if the new edge adds a node */
        ...
        for (t = e.type, i = 0; i < s.deg; i++) {
          x = s.edges[i];       /* traverse edges in current fragment */
          if ((x.mark >= 0)     /* if edge is in current fragment */
          &&  (d == ((s != x.src) ? x.src : x.dst))
          &&  (x.type > t))     /* if edge is parallel to ext. edge */
            t = x.type;         /* and has a larger type, adding it */
        }                       /* leads to a non-canonical result */
        if (e.type < t)         /* skip ext. edges with lower type */
          continue;             /* since the result is not canonical */
      ...
      if (this.src < this.frag.dst) { /* if source on rightmost path */
        x = this.emb.edges[this.path[this.pei]];
        t = (this.dir)          /* get code of downward path edge */
          ? (x.type << 1) +((s == x.src) ? 0 : 1) : x.type;
        u = (this.dir)          /* get edge code (type and dir.) */
          ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
        if (u <  t) continue;   /* skip smaller edge codes */
        if (u == t) {           /* if edge codes are equal */
          t = (x.src != s) ? x.src.type : x.dst.type;
  ...
  {                             /* --- create next extended fragment */
    ...
    int     t, u, i;            /* ext. edge type, loop variable */
    ...
    while (true) {              /* try to find another extension */
      ...
      while (this.dst > g.nodecnt) {     /* if there is none left */
        ...
        if (this.idx < 0)       /* if add. node or rightmost leaf, */
          ...
          u = (this.dir)        /* get edge code (type and direction) */
            ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
          if (t <  u) continue; /* skip smaller edge codes/types */
          if (t == u) {         /* for equal edge codes/types */
            d = (e.src != s) ? e.src : e.dst;
      ...
      if (this.dst < g.nodecnt){/* if the new edge closes a ring */
        ...
        s = g.nodes[this.src];  /* get the current source node */
        /* The following code rules out multigraphs, which are */
	/* possible if extensions are created from embeddings. */
        for (i = s.deg; --i >= 0; ) {
      ...
      else if (g.nodecnt >= this.max)
        continue;               /* if edge to new node, check size */
   ...
   *          -1, if the ring adaptation failed,
   *           0, if the ring adaptation succeeded,
   *              but the fragment needs to be modified,
  ...
  {                             /* --- compare two edges */
    ...
    /* Edge types can be compared directly, rather than edge codes,  */
    /* because this function is only needed in function adaptRing(). */
  ...
  {                             /* --- compare extension to fragment */
    Node d1, d2;                /* destination nodes */
    Edge e1, e2;                /* extension edges */
    int  t1, t2;                /* buffers for type/code comparisons */
    ...
    e1 =      this.edges[    0   ];      /* get extension edge */
    e2 = frag.list.edges[frag.idx];      /* in new and old fragment */
    d1 =      this.nodes[    1   ];      /* get destination edge */
    d2 = frag.list.nodes[frag.dst];      /* in new and old fragment */
    t1 = (this.dir)             /* get edge code (type and direction) */
       ? (e1.type << 1) +((d1 != e1.src) ? 0 : 1) : e1.type;
    t2 = (this.dir)             /* get edge code (type and direction) */
       ? (e2.type << 1) +((d2 != e2.src) ? 0 : 1) : e2.type;
    if (t1 < t2) return -1;     /* compare the codes */
  ...
  {                             /* --- check prefix words recursively */
    ...
    Edge e;                     /* to traverse/access the edges */
    Node s, d;                  /* to traverse/access the nodes */
    int  code;                  /* buffer for edge type */

    /* --- check for a complete code word --- */
    ...
    if (this.word[c] < cnt) {   /* if no new node is added */
      ...
      code = (this.dir)         /* get edge code (type and direction) */
           ? (e.type << 1) +((d != e.src) ? 0 : 1) : e.type;
      if (code < this.word[c+2]) return ei;  /* check the code */
      if (code > this.word[c+2]) return r;   /* of the edge */
  ...
  {                             /* --- create a graph's code word */
    ...
    s = new StringBuilder(nmgr.getName(k));
    ...
    for (i = 0; i < n; ) {      /* traverse the characters */
      ...
      k = this.word[++i];       /* get and decode the edge type */
      if (this.dir && ((k & 1) != 0)) s.append('<');
      s.append(emgr.getName((this.dir) ? k >> 1 : k));
      if (this.dir && ((k & 1) == 0)) s.append('>');
