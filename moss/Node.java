...
public class Node implements Comparable<Node>, Serializable {
  ...
  public static final int CHAIN    = TypeMgr.SPECIAL;
   ...
   *  (indirectly through <code>Arrays.sort()</code>).
  ...
  {                             /* --- sort the edges of a node */
    ...
    int  r, s;                  /* reversed flags of edges */

    if (this.deg > 12) {        /* if not very few edges to sort */
      Arrays.sort(this.edges, 0, this.deg, new Comparator<Edge> () {
        public int compare (Edge e1, Edge e2) {
          int  t1 = (e1.type << 1) +((e1.src == Node.this) ? 0 : 1);
          int  t2 = (e2.type << 1) +((e2.src == Node.this) ? 0 : 1);
          if (t1      > t2     ) return +1;
          if (t1      < t2     ) return -1;
    ...
    for (i = 0; ++i < this.deg; ) {
      ...
      if (e.src == this) { d = e.src.type; s =  e.type << 1; }
      else               { d = e.dst.type; s = (e.type << 1) +1; }
      do {                      /* traverse the preceding edges */
        x = this.edges[k-1];    /* (find the place for insertion) */
        r = (x.type << 1) +((x.src != this) ? 1 : 0);
        if (s >  r) break;      /* take edge direction into account */
        if (s == r) {           /* (even if edges are undirected) */
