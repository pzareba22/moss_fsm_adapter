 ...
 *  to the nodes and edges of a graph, namely by simply listing the
 *  nodes and edges (from the graph) that form the substructure.
 *  An embedding must contain at least one node, i.e., it cannot
 *  be empty. It need not contain any edges, though.</p>
 *  <p>If the edges array has a positive length, then usually
 *  <code>edges[edges.length-1]</code> contains the edge added last
 *  (perfect extensions and ring extensions may lead to exceptions
 *  from this rule, especially after adaptation).</p>
 *  <p>The nodes in the nodes array are usually sorted in the order
 *  in which they have been added in the extension process (perfect
 *  extensions and ring extensions may lead to exceptions from this
 *  rule, especially after adaptation).</p>
 *  @author Christian Borgelt
 *  @since  2002.03.11 */
/*--------------------------------------------------------------------*/
public class Embedding implements Serializable {
  ...
  /*  constants                                                       */
  /*------------------------------------------------------------------*/
  private static final long serialVersionUID = 0x00010002;
   ...
   *  <code>CONTAINED</code> in the class <code>Graph</code>, which
   *  is used as a special parameter and as a special return value
   *  for the function <code>Graph.embed()</code> in order to save
   *  a recursion parameter.</p>
   ...
   *  @param  graph the graph into which to embed
   ...
   *  The given node and edge arrays are copied.</p>
   *  @param  graph the graph referred to
   *  @param  nodes the nodes of the substructure
   *  @param  edges the edges of the substructure
   *  @since  2002.03.11 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected Embedding (Graph graph, Node[] nodes, Edge[] edges)
  {                             /* --- create embedding from arrays */
   ...
   *  needed as an embedding (in contrast to a fragment).</p>
   *  @param  cnf the canonical form holding the extension
   *  @since  2002.03.11 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- create an embedding */
    ...
    if (cnf.size > 0) {         /* if ring extension */
      for (i = 0; i < cnf.size; i++) {
        ...
        if (node.mark < 0) this.nodes[n++] = node;
        edge = cnf.edges[i];    /* traverse the extension edges */
        if (edge.mark < 0) this.edges[e++] = edge;
   ...
   *  It is only needed for <code>Embedding.extend()</code>.</p>
   *  @param  emb  the embedding to extend
   *  @param  edge the edge by which to extend the embedding
   ...
   *  function <code>Graph.hashCode()</code>.</p>
  ...
  {                             /* --- compute a hash code */
    ...
    for (n = 0, i = this.nodes.length; --i >= 0; ) {
      ...
      for (src.mark = 0, k = src.deg; --k >= 0; )
        if (src.edges[k].mark >= 0) src.mark++;
   ...
   *  @param  emb the embedding to compare to
   *  @return the number of edges that are common to the embeddings,
  ...
  {                             /* --- find length of common prefix */
    ...
    if (n > emb.edges.length) n = emb.edges.length;
    for (int i = 0; i < n; i++) /* find first edge that differs */
   ...
   *  source node (index in the embedding), the index of the destination
   *  node (which may be negative in order to indicate that the node is
   *  not yet part of the embedding), the type of the edge and the type
   *  of the destination node of the edge to add.</p>
   *  @param  src  the index of the source node
   *  @param  dst  the index of the destination node
   *               (or -1 if it is not yet part of the embedding)
   *  @param  edge the type of the extension edge
   *  @param  node the type of the destination node
   *  @return a list of created embeddings
   *  @since  2002.03.11 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- create extensions of an embed. */
    ...
    for (i = s.deg; --i >= 0; ) {
      ...
      if (e.type < edge) break;     /* compare the */
      if (e.type > edge) continue;  /* edge type */
      d = (e.src != s) ? e.src : e.dst;
      if (d.type < node) break;     /* compare destination */
      if (d.type > node) continue;  /* node type */
      if (d.mark != dst) continue;  /* and index */
      emb = new Embedding(this, e);
      emb.succ = list; list = emb;
   ...
   *  @param  harmful whether to check for a harmful overlap
   ...
   *  fragment and thus have the same number of nodes and edges.</p>
   *  @param  emb the embedding to check for an overlap
  ...
  {                             /* --- check for harmful overlap */
    ...
    for (i = n; --i >= 0; )     /* number the nodes of the subgraph */
      if (this.nodes[i].mark >= 0) this.nodes[i].mark = i;
    do {                        /* connected component marking loop */
      for (k = 0, i = m; --i >= 0; ) {
        ...
        if      (s < d) { e.dst.mark = s; k++; }
        else if (s > d) { e.src.mark = d; k++; }
