/*----------------------------------------------------------------------
  File    : CnFBreadth2.java
  Contents: breadth-first search spanning tree canonical form
            and corresponding maximum edge source extension
  Author  : Christian Borgelt
  History : 2009.07.06 file created from MaxSrcExt.java
            2009.08.03 most functions adapted (except ring mining)
            2010.01.22 renamed to CnFBreadth2.java (canonical form)
            2010.01.28 check of maximum degree of source node added
            2011.02.18 orbits used to suppress some equivalent siblings
 ...
 *  extended by adding an edge. Edges closing rings must lead
 ...
 *  <p>The difference to <code>CnFBreadth1</code> is the position of
 *  the destination node index in the precedence order of the edge
 *  properties. While in <code>CnFBreadth1</code> it is the last
 *  property to be compared, it is the second in this class.</p>
 *  @author Christian Borgelt
 *  @since  2009.07.06 */
/*--------------------------------------------------------------------*/
public class CnFBreadth2 extends CanonicalForm {
   ...
   *  @since  2009.07.06 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- initialize an extension */
    ...
    if (this.useOrbits()) {     /* if the node orbits are known */
   ...
   *  @since  2009.08.03 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- create the next extension */
    ...
    while (true) {              /* find the next unprocessed edge */
      ...
      if ((this.frag.idx >= 0)  /* if same source as preceding ext. */
      &&  (this.frag.src == this.src)) {
        t = this.frag.dst;      /* get previous destination index */
        if (this.dst <  t) continue;
        if (this.dst == t) {    /* skip smaller destination indices */
          x = this.emb.edges[this.frag.idx];
          t = (this.dir)        /* get code of previous ext. edge */
            ? (x.type << 1) +((s == x.src) ? 0 : 1) : x.type;
          u = (this.dir)        /* get edge code (type and dir.) */
            ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
          if ( (u <  t)         /* skip smaller edge types */
          ||  ((u == t) && (d.type < n[this.frag.dst].type)))
            continue;           /* and for equal edge types */
  ...
  {                             /* --- create next extended fragment */
    ...
    int     t, i;               /* edge type and destination index */
    boolean o;                  /* whether node orbits are known */
    ...
    while (true) {              /* try to find another extension */
      ...
      while (this.dst > g.nodecnt) {     /* if there is none left */
        ...
        if (s.deg > 0) {        /* if there are incident edges, */
          ...
          if (d.mark > this.src) i = d.mark;
        }                       /* ensure a greater destination index */
        /* An extension edge from the node that was already extended  */
        /* in the previous step is admissible only if its description */
        /* is lexicographically no smaller than the description of    */
        /* any previous extension edge from the same node.            */
      ...
      if (this.dst >= g.nodecnt) {
        if (g.nodecnt >= this.max) continue;
        break;                  /* if the edge leads to a new node, */
      }                         /* check against the maximum size */
      if (g.nodes[this.dst].type == this.xemgr.getDest())
        break;                  /* if the destination type fits, */
  ...
  {                             /* --- compare extension to fragment */
    ...
    t1 = (!this.dir)            /* get edge code (type and direction) */
       ? e1.type : (e1.type << 1) +((d1 != e1.src) ? 0 : 1);
    t2 = (!this.dir)            /* get edge code (type and direction) */
       ? e2.type : (e2.type << 1) +((d2 != e2.src) ? 0 : 1);
  ...
  {                             /* --- construct (part of) code word */
    ...
    for (i = k = 0; i < n; i++) {
      ...
      if (e.src.mark < e.dst.mark) {
        ...
        this.word[++k] = e.dst.mark; /* start with source node */
  ...
  {                             /* --- check prefix words recursively */
    ...
    for (i = 0; i < s.deg; i++) {
      ...
      if (k      < this.word[c+1]) return ei;  /* check the index */
      if (k      > this.word[c+1]) continue;   /* of the dest. node */
      code = (!this.dir)        /* get edge code (type and direction) */
           ? e.type : (e.type << 1) +((s == e.src) ? 0 : 1);
  ...
  {                             /* --- construct canonic code word */
    ...
    for (i = 0; i < s.deg; i++) {
      ...
      if (k      < this.word[c+1]) {
        this.word[c+1] = k;     /* set new destination node index */
        this.word[c+2] = Integer.MAX_VALUE;
      }                         /* on change invalidate next entry */
      code = (!this.dir)        /* get edge code (type and direction) */
           ? e.type : (e.type << 1) +((s == e.src) ? 0 : 1);
      if (code   > this.word[c+2])
        return changed;         /* compare the edge code */
      if (code   < this.word[c+2]) {
