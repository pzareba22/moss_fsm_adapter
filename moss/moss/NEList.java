/*----------------------------------------------------------------------
  ...
  History : 2007.06.22 file created
            ...
            2012.01.23 bug in node/edge type output fixed
...
public class NEList extends FreeNtn {
  ...
  {                             /* --- read an integer number */
    ...
    if ((c < '0') || (c > '9')) /* check for at least one digit */
      throw new IOException("digit expected instead of '"
                            +(char)c +"' (" +c +")");
  ...
  {                             /* --- read a node or edge label */
    ...
    StringBuilder s;            /* buffer for the type name */
    ...
    s = new StringBuilder();    /* create a buffer for the type name */
    while ((c >= 0) && (c != '\n')) {
  ...
  {                             /* --- parse a graph description */
    ...
    while (true) {              /* read loop for nodes and edges */
      ...
      if ((c == 'n') || (c == 'v')) {
        ...
        if (c < 0) throw new IOException("invalid node type " +t);
        if (graph == null)      /* if it does not yet exist, */
          graph = new Graph(this);         /* create a graph */
        graph.addNode(c); }     /* add a node to the graph */
      else if ((c == 'a') || (c == 'd') || (c == 'e')) {
        ...
        if (c < 0) throw new IOException("invalid edge type " +t);
        if (graph == null)      /* create a graph if necessary and */
          graph = new Graph(this);     /* add an edge to the graph */
        graph.addEdge(s-1, d-1, c); }
      else if (c == '%') {      /* if comment line, */
        do { c = this.read(); } /* skip the rest of the line */
        while ((c >= 0) && (c != '\n')); }
  ...
  {                             /* --- create a string description */
    if (this.linog == null)     /* create a LiNoG notation if needed */
      this.linog = new LiNoG(this.nodemgr, this.edgemgr);
    return this.linog.describe(graph);
  }  /* describe() */           /* create a LiNoG description */
  ...
  {                             /* --- write a graph description */
    ...
    for (i = 0; i < graph.nodecnt; i++) {
      ...
      t = n.getBaseType();      /* get the destination node type */
      if (graph.coder != null) t = graph.coder.decode(t);
    ...
    for (i = 0; i < graph.edgecnt; i++) {
      ...
      writer.write(this.edgemgr.getName(e.getBaseType()));
   ...
   *  using the function <code>write()</code>.</p>
  ...
  {                             /* --- main function for testing */
    ...
    try {                       /* try to parse the description */
      ...
      Graph    graph  = ntn.parse(reader); reader.close();
      Writer   writer = new OutputStreamWriter(System.out);
      ntn.write(graph, writer); /* write the parsed graph and */
      writer.flush(); }         /* flush the writer afterwards */
