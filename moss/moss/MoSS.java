...
public class MoSS extends JFrame
  ...
  /*  constants                                                       */
  /*------------------------------------------------------------------*/
  private static final long serialVersionUID = 0x00030000;
  /** the version string */
  ...
  {                             /* --- create toolbox dialog */
    ...
    this.fn_data = tab.addFileInput("moss.dat");
    ...
    this.fn_sub = tab.addFileInput("moss.sub");
    ...
    tab.addHelp(
      ...
      +"GUI version " +MoSS.VERSION +", Miner " +Miner.VERSION +"\n"
  ...
  {                             /* -- get and store a file name */
    this.getFileChooser().setDialogTitle("Choose " +title +" File...");
   ...
   *  @param  file the identifier output file to set
   *  @since  2014.09.25 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
  ...
  {                             /* --- load configuration */
    if (file == null) {         /* if no file name is given */
      this.getFileChooser().setDialogTitle("Load Configuration...");
      int r = this.chooser.showOpenDialog(this);
      if (r != JFileChooser.APPROVE_OPTION) return;
    ...
    try {                       /* open the configuration file */
      ...
      this.fn_data.setText(this.readLine(reader));
      this.fn_sub.setText (this.readLine(reader));
      this.fn_ids.setText (this.readLine(reader));
      ...
      this.minsize.setValue(Integer.valueOf(this.readInt(reader)));
      this.maxsize.setValue(Integer.valueOf(this.readInt(reader)));
      this.minring.setValue(Integer.valueOf(this.readInt(reader)));
      this.maxring.setValue(Integer.valueOf(this.readInt(reader)));
      this.level.setValue  (Integer.valueOf(this.readInt(reader)));
      this.maxepg.setValue (Integer.valueOf(this.readInt(reader)));
  ...
  {                             /* --- save configuration */
    if (file == null) {         /* if no file name is given, */
      this.getFileChooser().setDialogTitle("Save Configuration...");
      int r = this.chooser.showOpenDialog(this);
      if (r != JFileChooser.APPROVE_OPTION) return;
      file = this.chooser.getSelectedFile();
    }                           /* let the user choose a file */
    try {                       /* open the configuration file */
      ...
      writer.write(this.fn_data.getText()); writer.write('\n');
      writer.write(this.fn_sub.getText());  writer.write('\n');
      writer.write(this.fn_ids.getText());  writer.write('\n');
  ...
  /*------------------------------------------------------------------*/
  /** Show a molecule/substructure/identifier table.
  ...
  {                             /* --- show a molecule table */
    ...
    if      (text == this.fn_ids) { /* if identifier file, */
      mode = MoSSTable.IDS;     /* get the identifier mode */
      msg  = null; }            /* clear the format string */
    else if (text == this.fn_sub) { /* if substructure file */
    ...
    if (mode != MoSSTable.SUBS)
      view.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
  ...
  {                             /* --- execute substructure search */
    ...
    try {                       /* and try to initialize it */
      ...
      catch (Exception e) { split= 0.5F; } /* get threshold value */
      this.thresh.setText(String.valueOf(split));
      this.miner.setGrouping(split, this.invert.isSelected());

      fname = this.fn_data.getText();
      ...
      this.miner.setOutput(fname, fmt, this.fn_ids.getText());
      ...
      catch (Exception e) { supp = 10.0F; } /* min. support in focus */
      this.minsupp.setText(String.valueOf(supp));
      try { comp = Math.abs(Float.parseFloat(this.maxsupp.getText())); }
      catch (Exception e) { comp = 2.0F; }  /* max. support in compl. */
      this.maxsupp.setText(String.valueOf(comp));
  ...
  {                             /* --- show result of computation */
    ...
    Throwable e;                /* error status of the search */
