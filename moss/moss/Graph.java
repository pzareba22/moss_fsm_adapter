/*----------------------------------------------------------------------
  ...
  History : 2002.03.11 file created (as Molecule.java)
            ...
            2020.10.04 some utility functions added (for debugging)
            2021.10.17 direction handling added to function embed()
----------------------------------------------------------------------*/
package moss;
...
public class Graph implements Cloneable, Serializable {
  ...
  /*  constants                                                       */
  /*------------------------------------------------------------------*/
  private static final long serialVersionUID = 0x00010006;
  ...
  /** a dummy parameter/return value for getting one embedding
   *  (saves a recursion parameter) */
  ...
  { this(ntn, null, 16, 16, false); }

  /*------------------------------------------------------------------*/
  /** Create a (empty) graph with default array sizes.
   *  @param  ntn     the notation of the graph
   *  @param  coder   the node type recoder of the graph
   *  @since  2010.01.21 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public Graph (Notation ntn, Recoder coder)
  { this(ntn, coder, 16, 16, false); }

  /*------------------------------------------------------------------*/
  /** Create a (empty) graph with default array sizes.
   *  @param  ntn     the notation of the graph
   *  @param  coder   the node type recoder of the graph
   *  @param  dir     whether the graph is directed
   *  @since  2021.10.07 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public Graph (Notation ntn, Recoder coder, boolean dir)
  { this(ntn, coder, 16, 16, dir); }
  ...
  { this(ntn, null, nodecnt, edgecnt, false); }
   ...
   *  @param  edgecnt the expected number of edges
   *  @param  dir     whether the graph is directed
   *  @since  2022.09.27 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public Graph (Notation ntn, int nodecnt, int edgecnt, boolean dir)
  { this(ntn, null, nodecnt, edgecnt, dir); }
  ...
  { this(ntn, coder, nodecnt, edgecnt, false); }
  ...
  public Graph (Notation ntn, Recoder coder,
                int nodecnt, int edgecnt, boolean dir)
  {                             /* --- create an empty graph */
    this.dir     = dir;         /* default: undirected graph */
   ...
   *  @param  reversed  whether the edge is reversed
   *  @since  2010.01.23/2003.08.04 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected Graph (Graph graph, int src, int dst,
                   int edge, int node, boolean reversed)
  {                             /* --- create an extended graph */
    ...
    this.dir     = graph.dir;   /* copy directionality, */
    this.ntn     = graph.ntn;   /* the notation and the recoder */
    ...
    if (src >= 0) {             /* if to add an edge */
      ...
      if (reversed) { i = src; src = dst; dst = i; }
    ...
    if (frag.chcnt <= 0) {      /* if there are no chains */
      ...
      for (i = emb.edges.length; --i >= 0; ) {
        ...
        this.edges[i] = new Edge(this.nodes[edge.src.mark],
                                 this.nodes[edge.dst.mark],
                                 edge.type, edge.flags);
   ...
   *  <p>For this function to work properly, the argument graph
   ...
   *  <p>Even if the graph contains a node type recoder,
   *  the node type is stored exactly as it is given.</p>
   ...
   *  <p>If the graph contains a node type recoder,
   *  the given node type is automatically encoded with it.</p>
   *  @param  type the type of the node to add
   *  @return the index of the node in the graph
   *  @see    #addNode(int)
  ...
  {                             /* --- add a node to a graph */
    if (this.coder != null) type = this.coder.encode(type);
  ...
  /*------------------------------------------------------------------*/
  /** Get the type of a node.
   *  <p>Even if the graph contains a recoder for the node types,
   *  the node type is not decoded, but returned as it is stored.</p>
   *  @param  index the index of the node
   *  @return the type of the <code>index</code>-th node of the graph
   *  @see    #getNodeType(int)
   *  @since  2010.01.19 (Christian Borgelt) */
  ...
  { return this.nodes[index].type; }  /* --- get the type of a node */

  /*------------------------------------------------------------------*/
  /** Get the marker of a node.
   *  @param  index the index of the node
   *  @return the marker of the <code>index</code>-th node of the graph
   *  @since  2020.10.04 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public int getNodeMark (int index)
  { return this.nodes[index].mark; }

  /*------------------------------------------------------------------*/
  /** Get the name of a node.
   *  <p>If the graph contains a recoder for the node types,
   *  the node type is automatically decoded. Then the node name
   *  is retrieved from the node type manager of the notation.</p>
   *  @param  index the index of the node
   *  @return the name of the <code>index</code>-th node of the graph
   *  @since  2020.10.04 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public String getNodeName (int index)
  { return this.ntn.nodemgr.getName(this.getNodeType(index)); }
   ...
   *  <p>Since edge types are currently not encoded,
   *  there is no need to distinguish <code>getEdgeType</code>
   *  and <code>getEdgeTypeRaw</code>.
   ...
   *  @param  index the index of the edge
   *  @return the marker of the <code>index</code>-th edge of the graph
   *  @since  2020.10.04 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public int getEdgeMark (int index)
  { return this.edges[index].mark; }

  /*------------------------------------------------------------------*/
  /** Get the name of an edge.
   *  <p>Then the edge name is retrieved from the edge type manager
   *  of the notation.</p>.
   *  @param  index the index of the edge
   *  @return the name of the <code>index</code>-th edge of the graph
   *  @since  2020.10.04 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public String getEdgeName (int index)
  { return this.ntn.edgemgr.getName(this.edges[index].type); }
   ...
   *  <p>In a simple graph any two node are connected
   *  by at most one edge, not multiple edges.</p>
   *  <p>This function indexes the nodes of the graph.</p>
   *  @return whether the graph is simple
   *  @since  2021.10.06 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public boolean simple ()
  {                             /* --- check whether graph is simple */
    Node node;                  /* to traverse the nodes of the graph */
    Edge edge;                  /* to traverse the edges of the graph */
    int  adj;                   /* to traverse adjacent node indices */
    int  occs[] = new int[this.nodecnt];    /* node occurrence flags */
    ...
    for (int i = 0; i < this.nodecnt; i++) {
      ...
      for (int k = node.deg; --k >= 0; ) {
        edge = node.edges[k];   /* traverse the incident edges */
        adj  = (edge.dst == node) ? edge.src.mark : edge.dst.mark;
        if ((adj >= 0) && (occs[adj] > i))
          return false;         /* if destination did already occur, */
        occs[adj] = i+1;        /* there is a duplicate edge, */
      }                         /* otherwise mark the node occurrence */
    }                           /* (incremental marker) */
    return true;                /* no duplicate edges: simple graph */
  }  /* simple() */
   ...
   *                 (<code>false</code>)
   *  @return whether nodes were removed in the trimming process
  ...
  {                             /* --- trim graphs */
    ...
    for (i = this.nodecnt; --i >= 0; ) {
      ...
      if (node.mark <= Integer.MIN_VALUE) continue;
      for (k = n = 0; k < node.deg; k++) {
        edge = node.edges[k];   /* traverse the edges of the node */
        if (edge.mark > Integer.MIN_VALUE) node.edges[n++] = edge;
    ...
    for (i = n = 0; i < this.nodecnt; i++) {
      ...
      if (node.mark > Integer.MIN_VALUE) this.nodes[n++] = node;
    }                           /* remove the deleted nodes */
    this.nodecnt = n;           /* set the new node counter */
    for (i = n = 0; i < this.edgecnt; i++) {
      edge = this.edges[i];     /* traverse the edges */
      if (edge.mark > Integer.MIN_VALUE) this.edges[n++] = edge;
   ...
   *  the nodes and edges contained in it. It starts from a node
   ...
   *                <code>CONTAINED</code> for a containment check or
   *                the constant <code>MARKONE<code> for marking only
   *                one (arbitrary) embedding
  ...
  {                             /* --- embed a graph recursively */
    int       i, m, t;          /* loop variable, node type buffers */
    int       c, code;          /* edge codes (type and direction) */
    Edge      e, edge, x;       /* to access/traverse the edges */
    Node      s, d, src, dst;   /* to access/traverse the nodes */
    Node      end;              /* end of a chain (if embedded) */
    boolean   chn;              /* flag for a chain to embed/match */
    Embedding emb;              /* created embedding */

    if (eid >= graph.edgecnt) { /* if all edges have been matched */
      if ((list == CONTAINED)   /* if only to check for containment */
      ||  (list == MARKONE))    /* (maybe marking it in nodes/edges), */
        return CONTAINED;       /* return a special dummy embedding */
      emb = new Embedding(this, ens, ees);
      emb.succ = list;          /* create a new embedding */
      return emb;               /* and add it at the head */
    }                           /* of the embedding list */
    e = graph.edges[eid];       /* get the next edge to embed */
    s = e.src; d = e.dst;       /* and its incident nodes */
    if ((s.mark < 0) || ((d.mark >= 0) && (s.mark > d.mark))) {
      s = e.dst; d = e.src; }   /* identify source and dest. node */
    c = (this.dir)              /* get edge code (type and direction) */
      ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
    /* The traversal direction of the edge for embedding is s -> d, */
    /* but this may or may not coincide with the edge direction,    */
    /* which is e.src -> e.dst. This may have to be checked later.  */
    src = ens[s.mark];          /* get image of source node */
    m   = d.mark;               /* note the dest. node marker and */
    t   = d.type & ~Node.CHAIN; /* its type without the chain flag */
    chn = d.isChain();          /* and check for a chain node */
    emb = ((list == CONTAINED) || (list == MARKONE)) ? null : list;
    for (i = src.deg; --i >= 0; ) {
      edge = src.edges[i];      /* traverse the unmarked edges */
      if (edge.mark >= 0) continue;
      code = (this.dir)         /* get edge code (type and directioN) */
           ? (edge.type << 1) +((src == edge.src) ? 0 : 1) : edge.type;
      if (code > c) continue;   /* compare the */
      if (code < c) return emb; /* edge code/type */
      dst = (edge.src == src) ? edge.dst : edge.src;
      /* The traversal direction of the target edge is src -> dst, */
      /* but this may or may not coincide with the edge direction, */
      /* which is edge.src -> edge.dst. */
      if (d.type != Node.WILDCARD) { /* if it is not a wildcard */
        if (dst.type > t) continue;
        if (dst.type < t) return emb;
      }                         /* compare destination node type */
      if (dst.mark != d.mark) continue;  /* compare dest. index */
      if      (!chn) {          /* if not to match/embed a chain */
        if (dst.deg < d.deg)  continue;
        ...
        end = (x.src != d) ? x.src : x.dst;
        end.mark = nid;         /* mark the node after the chain */
      }                         /* in the graph to embed */
      ees[edge.mark = eid] = edge;  /* mark and store image edge */
      if (m < 0) {                  /* as well as image node */
        ens[dst.mark = d.mark = nid++] = dst; }
      emb = this.embed(graph, ens, nid, ees, eid+1, list);
      if ((emb  != null)        /* find embeddings recursively */
      &&  (list == MARKONE))    /* if only to mark one embedding, */
        break;                  /* abort the embedding search */
      if (m < 0) {              /* unmark original of destination */
        dst.mark = d.mark = -1; nid--; }
      if (end != null) {        /* unmark chain edge, end of chain */
        ees[--eid].mark = end.mark = -1; }
      edge.mark = -1;           /* unmark the followed edge */
      if (emb == CONTAINED)     /* if only to check containment, */
        break;                  /* check the recursion result */
      if (emb != null) list = emb;
  ...
  {                             /* --- find embeddings of a graph */
    ...
    if (graph.nodecnt <= 1) {   /* if there is only one node */
      ...
      if (t == Node.WILDCARD) return CONTAINED;
      for (i = this.nodecnt; --i >= 0; ) {
        if (this.nodes[i].type == t) {
          if (list == MARKONE)  /* if the node type was found */
            this.nodes[i].mark = 0;
          return CONTAINED;     /* mark the image node as root */
        }                       /* and return that the given graph */
      }                         /* is contained in this graph */
      return null;              /* otherwise return an empty list */
    }
    ...
    for (i = this.nodecnt; --i >= 0; ) {
      ...
      if (((t != Node.WILDCARD) && (node.type != t))
      ...
      if ((emb  != null)        /* match the edges recursively */
      &&  (list == MARKONE))    /* if only to mark one embedding, */
        break;                  /* abort the embedding search */
      node.mark = -1;           /* unmark the root node */
      if (emb == CONTAINED)     /* if only to check containment, */
        break;                  /* check the recursion result */
      if (emb != null) list = emb;
    }                           /* transfer the current embeddings */
    if ((emb == null) || (list != MARKONE))
   ...
   *  <p>This function embeds a given graph in one possible way and
   *  and marks the corresponding embedding (if one exists). For this
   *  function to work properly, the graph to embed must either have
   *  been processed with the function <code>prepareEmbed()</code>,
   *  or it must have been created from a fragment that itself was
   *  generated in the search process (class <code>Miner</code>),
   *  so that the edges are sorted in a specific way (so that source
   *  nodes are always already numbered). In addition, the graph
   ...
   *  <code>prepare()</code> on it.</p>
   *  @param  graph the graph to embed
   *  @return the constant <code>CONTAINED</code> or <code>null</code>
   *  @since  2020.10.03 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public Embedding markEmbed (Graph graph)
  { return (graph.nodecnt <= 0) ? null : this.embed(graph, MARKONE); }

  /*------------------------------------------------------------------*/
  /** Check whether a node type is contained in this graph.
   *  @param  type the type of the node to check for
   *  @return a list of embeddings of the node
   *  @since  2010.01.21 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected boolean contains (int type)
  {                             /* --- check occurrence of node type */
    for (int i = this.nodecnt; --i >= 0; )
      if ((type == Node.WILDCARD) || (this.nodes[i].type == type))
   ...
   *  <code>embed(Graph)</code>, which tries to find all embeddings).
   ...
   *  calling the function <code>prepare()</code> on it.</p>
   *  @param  graph the graph to embed
  ...
  {                             /* --- check whether a graph is cont. */
    if (graph.nodecnt <= 0) return true;
    if (graph.nodecnt <= 1) return this.contains(graph.nodes[0].type);
   ...
   *  canonical form that is specified by a given canonical form.</p>
   ...
   *              in the first <code>fixed</code> edges,
   ...
   *  into the order that yields the smallest code word w.r.t.
   *  a given canonical form.</p>
   *  @param  cnf the extension object defining the canonical form
  ...
  {                             /* --- make a graph canonic */
    if (!cnf.makeCanonic(this, keep)) return false;
   ...
   *  <p>This function brings the nodes and edges of the graph into the
   *  order that yields the smallest code word w.r.t. a given canonical
   *  form. It also sorts all edges per node accordingly, so that the
   ...
   *  with one of the functions <code>CanonicalForm.makeCanonic()</code>
   *  or <code>CanonicalForm.adaptRing()</code> and was stored in
   *  internal arrays of the canonical form.</p>
   *  @param  cnf the canonical form providing the map
   *  @since  2006.10.24 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  protected void map (CanonicalForm cnf)
  {                             /* --- reorder edges and nodes */
    if (this.edgecnt <= 0) return;
   ...
   *  graph (the graph the function is called on and the argument
   *  graph), as they are implicitly represented by the order
   *  of their nodes and edges, are equal.</p>
   *  <p>This function is equivalent to <code>equals(Object)</code>
   *  provided the two graphs compared have both been made canonical
   *  with the function <code>makeCanonic(CanonicalForm)</code>.
  ...
  {                             /* --- compare (canonical) code words */
    int  i;                     /* loop variable */
    Node s1, s2, d1, d2;        /* to traverse the nodes */
    Edge e1, e2;                /* to traverse the edges */

    if (graph == this) return true;   /* check some trivial cases */
    if (graph == null) return false;  /* to ensure some properties */
    if ((this.nodecnt != graph.nodecnt)
    ||  (this.edgecnt != graph.edgecnt))
      return false;             /* compare node and edge counters */
    if (this.nodecnt <= 0) return true;
    if (this.nodes[0].type != graph.nodes[0].type)
      return false;             /* compare the first letter (root) */
    for (i = 0; i < this.nodecnt; i++)  /* mark all nodes */
    ...
    for (i = 0; i < this.edgecnt; i++) {
      ...
      if (e1.src.mark < e1.dst.mark) { s1 = e1.src; d1 = e1.dst; }
      else                           { s1 = e1.dst; d1 = e1.src; }
      if (e2.src.mark < e2.dst.mark) { s2 = e2.src; d2 = e2.dst; }
      else                           { s2 = e2.dst; d2 = e2.src; }
      if ((s1.type != s2.type) || (s1.mark != s2.mark)
      ||  (d1.type != d2.type) || (d1.mark != d2.mark))
        return false;           /* compare the node types */
    }                           /* and the node indices */
    for (i = 0; i < this.nodecnt; i++)  /* unmark all nodes */
   ...
   *  @param  ntn  the notation in which to create the description
   *  @return a string description of the graph
   *  @since  2007.06.29 (Christian Borgelt) */
  /*------------------------------------------------------------------*/
   ...
   *  function <code>makeCanonic(CanonicalForm)</code> on it.</p>
   *  @param  cnf the canonical form
  ...
  {                             /* --- show the node orbit reps. */
    int           i, k;         /* loop variable, buffer */
    StringBuilder s;            /* created description */
    TypeMgr       t;            /* node type manager */
    Node          node;         /* to traverse the nodes */

    t = this.getNodeMgr();      /* get the node type manager and */
    s = new StringBuilder();    /* create an empty string buffer */
    for (i = 0; i < this.nodecnt; i++) {
      node = this.nodes[i];     /* traverse the nodes of the graph */
      if (i > 0) s.append(' '); /* print a separator if necessary */
  ...
  {                             /* --- main function for testing */
    ...
    try {                       /* try general line notation */
      ...
      if (graph != null)
        System.err.println(graph.simple());
