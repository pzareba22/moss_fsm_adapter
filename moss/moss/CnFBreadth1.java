/*----------------------------------------------------------------------
  ...
  History : 2002.03.11 file created as file submol.java
            ...
            2021.10.07 bug in size limit check in nextFrag() fixed
            2021.10.08 adapted plain    search mode to directed edges
...
public class CnFBreadth1 extends CanonicalForm {
  ...
  {                             /* --- create the next extension */
	...
	this.dst = d.mark;      /* note the destination node index */
      ...
      &&  (this.frag.src == this.src)) {
        ...
        if (u == t) {           /* and smaller node types */
          t = n[this.frag.dst].type;
          if ( (d.type <  t)    /* get type of previous dest. node */
          ||  ((d.type == t) && (this.dst < this.frag.dst)))
            continue;           /* compare dest. node type and */
  ...
  {                             /* --- create next extended fragment */
    ...
    int     t, u, i;            /* node/edge type and dest. index */
        ...
        if (s.deg > 0) {        /* if there are incident edges, */
          ...
          if (d.mark > this.src) {  /* if last edge leads downward */
            u = (this.dir)      /* get edge code (type and dir.) */
              ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
            if (t <  u) continue;
            if (t == u) {       /* skip smaller edge types */
  ...
  {                             /* --- construct (part of) code word */
    ...
    for (i = k = 0; i < n; i++) {
      ...
      if (e.src.mark < e.dst.mark) { /* if "forward" edge */
  ...
  {                             /* --- check prefix words recursively */
    ...
    for (i = 0; i < s.deg; i++) {
      ...
      code = (this.dir)         /* get edge code (type and direction) */
           ? (e.type << 1) +((s == e.src) ? 0 : 1) : e.type;
      if (code   < this.word[c+1]) return ei;  /* check the code */
      if (code   > this.word[c+1]) return r;   /* of the edge */
      d = (e.src != s) ? e.src : e.dst;
