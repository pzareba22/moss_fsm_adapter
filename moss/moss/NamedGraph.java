/*----------------------------------------------------------------------
  ...
  History : 2002.03.11 file created created as Graph.java
            ...
            2021.10.07 handling of directed graphs added
----------------------------------------------------------------------*/
package moss;
...
public class NamedGraph extends Graph
  ...
  public NamedGraph (Notation ntn, String name, float value,
                     int group, boolean dir)
  { this(ntn, 16, 16, name, value, group, dir); }
  ...
  public NamedGraph (Notation ntn, int nodecnt, int edgecnt,
                     String name, float value, int group, boolean dir)
  {                             /* --- create an empty graph */
    super(ntn,nodecnt,edgecnt,dir); /* initialize the graph */
    this.name  = name;          /* note the graph name, */
    this.value = value;         /* the associated value, */
    this.group = group;         /* and the group flag */
    this.succ  = null;          /* there is no list */
  ...
  {                             /* --- split into connected comps. */
    ...
    while (--ncc >= 0) {        /* traverse the connected components */
      ...
      g = new NamedGraph(this.ntn, n-i, m-k,
                         this.name, this.value, this.group, this.dir);
  ...
  {                             /* --- produce a Prolog description */
    ...
    StringBuilder s;            /* buffer for Prolog string */

    ntmgr = this.ntn.getNodeMgr();
    etmgr = this.ntn.getEdgeMgr();
    s = new StringBuilder();    /* create buffer for description */
