...
public class OverlapGraph {
   ...
   *  @throws IOException if parsing the graph failed
   *  @since  2007.06.18 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  public static OverlapGraph parse (Reader reader)
    throws IOException
  {                             /* --- parse ASCII DIMACS format */
    int           i, k;         /* loop variable */
    int           c;            /* character read */
    StringBuilder buf;          /* buffer for a field */
    ...
    buf = new StringBuilder();  /* read the next field */
  ...
  {                             /* --- main function for testing */
    ...
    if (args.length <= 1) {     /* if only one argument is given */
      ...
      try { olap = OverlapGraph.parse(new FileReader(args[0])); }
